...
namespace 服务端Test.IBLL
{
    ...
    internal class B基本状态 : IB基本状态
    {
        ...
        private M对象 创建通用访问对象()
        {
            ...
            __对象.添加属性("连接设备", () => HJSON.序列化(_缓存.连接设备, new IPAddressConverter()), E角色.所有, null);
            __对象.添加属性("业务状态", () => HJSON.序列化(_缓存.业务状态, new DictionaryConverter<string, string>()), E角色.所有, null);
            #endregion
